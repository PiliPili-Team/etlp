...
interface LogResponse { lines: string[]; next_bytes: number; }
interface LogPaths    { app_log: string; mpv_log: string | null; }
...
function parseLine(raw: string): ParsedLine {
    ...
    if (m2) {
        return { time: "", level: m2[1].toLowerCase() as ParsedLine["level"], content: m2[2] };
    }

    // Level keyword anywhere near the start
    const m3 = raw.match(/\b(INFO|WARN|ERROR|DEBUG|TRACE)\b(.*)$/i);
    if (m3) {
        return { time: "", level: m3[1].toLowerCase() as ParsedLine["level"], content: m3[2].trim() || raw };
    }
...
function LogLineView({ raw }: { raw: string }) {
    ...
    if (p.level === "raw") {
        return <div className="log-line"><span className="log-content">{raw}</span></div>;
    }
    return (
        <div className="log-line">
            {p.time && <span className="log-time">{p.time.slice(11)}</span>}
            <span className={`log-badge log-badge-${p.level}`}>{p.level.toUpperCase()}</span>
...
export default function Logs() {
    ...
    useEffect(() => {
        invoke<LogPaths>("get_log_paths").then(setPaths).catch(() => {});
    }, []);

    const fetchChunk = useCallback(async (since: number): Promise<number> => {
        try {
            const resp = await invoke<LogResponse>("get_log_lines", { sinceBytes: since });
    ...
    useEffect(() => {
        ...
        const iv = setInterval(async () => {
            if (!active.ok) { clearInterval(iv); return; }
            const next = await fetchChunk(pos);
            if (active.ok) { pos = next; posRef.current = next; }
        }, POLL_MS);

        return () => { active.ok = false; clearInterval(iv); };
    }, [source, fetchChunk]);
            ...
            <div className="log-container">
                <div className="log-toolbar">
                    ...
                    <div style={{ display: "flex", gap: 7, alignItems: "center", flexShrink: 0 }}>
                        <input
                            className="input"
                            style={{ minWidth: 110, height: 28, padding: "3px 10px", fontSize: 12 }}
                            placeholder={t("logs_filter")}
                            value={filter}
                            onChange={(e) => setFilter(e.target.value)}
                        />
                        <button className="btn" style={{ padding: "4px 10px", fontSize: 12 }}
                            onClick={() => { setLines([]); posRef.current = 0; }}>
                            {t("logs_clear")}
                        </button>
                        {!autoScroll && (
                ...
                <div className="log-body" ref={bodyRef} onScroll={handleScroll}>
                    {displayed.length === 0 ? (
                        <span style={{ color: "var(--text-3)" }}>
                            {source === "mpv" && !hasMpv ? t("logs_no_mpv") : t("logs_empty")}
                        </span>
                    ) : (
