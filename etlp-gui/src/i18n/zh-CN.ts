export const zhCN = {
    ...
    sys_trakt_sync_note:
        "播放结束后自动同步：看完本集（约 90% 及以上）即标记为已观看并写入历史，Trakt 的「继续观看」随之显示下一集；同时还会按真实进度补发一次 scrobble 作为双保险（约 80% 及以上记为已观看，未达则保存为续播点）；此前在媒体服务器已看完的本季剧集会一并补记并自动去重；重温本集会再次写入观看记录，是否受时间间隔限制取决于下方「允许重复标记」开关",
