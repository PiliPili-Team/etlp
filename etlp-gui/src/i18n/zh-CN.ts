export const zhCN = {
    ...
    sys_trakt_sync_note:
        "播放结束后自动同步，两种方式按进度分工、互不重复：看完本集（约 90% 及以上）写入观看历史，Trakt 的「继续观看」随之显示下一集；未看完则按真实进度发送一次 scrobble（Trakt 在约 80% 以上仍记为已观看，否则保存为续播点）。此前在媒体服务器已看完的本季剧集会一并补记并自动去重；重温本集会再次写入观看记录，是否受时间间隔限制取决于下方「允许重复标记」开关",
