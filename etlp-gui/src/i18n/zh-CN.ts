export const zhCN = {
    ...
    sys_trakt_sync_note:
        "播放结束后会自动把观看情况同步到 Trakt：基本看完（约 80% 以上）就标记为「已看」，并在「继续观看」里显示同剧的下一集；只看了一部分则记住播放位置，方便下次接着看。之前在媒体服务器里已经看过的同季其他剧集也会一起标记，已标记过的不会重复。重看同一集会再记录一次，是否受时间间隔限制由下方的「允许重复标记」开关控制",
    ...
    sys_bangumi_sync_note:
        "播放结束后自动同步：Bangumi 没有单集进度的概念，因此本集只要有效观看（约 20 秒以上、非误点即退）即标记为「看过」，整个条目置为「在看」；媒体服务器中此前已看完的本季剧集会一并补标",
    ...
    cfg_update_available: "发现新版本 v{version}，正在打开下载页",
