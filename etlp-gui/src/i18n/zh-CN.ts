export const zhCN = {
    ...
    sys_trakt_sync_note:
        "播放结束后自动同步：本集有效观看（约 20 秒以上、非误点即退）即上报，由 Trakt 按进度判定——约 80% 及以上标记为已观看并写入历史（「继续观看」随之显示下一集），未达则保存为续播进度；此前在媒体服务器已看完的本季剧集会一并补记并自动去重；重温本集会再次标记，是否受时间间隔限制取决于下方「允许重复标记」开关",
