export const zhCN = {
    ...
    sys_trakt_sync_note:
        "播放结束后自动同步：本集需有效观看（约 20 秒以上、非误点即退）才会上报，时长过短会跳过；看到约 80% 及以上标记为已观看并写入历史，未达则按真实进度保存为续播点（可在 Trakt 的 Up Next 跨设备续播，注意 Trakt 要求进度高于 1% 才会保存）；媒体服务器中此前已看完的本季剧集会一并补记",
