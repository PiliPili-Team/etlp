...
export const zhTW: typeof zhCN = {
    ...
    sys_trakt_sync_note:
        "播放結束後自動同步：本集有效觀看（約 20 秒以上、非誤點即退）即上報，由 Trakt 按進度判定——約 80% 及以上標記為已觀看並寫入記錄（「繼續觀看」隨之顯示下一集），未達則儲存為續播進度；此前在媒體伺服器已看完的本季劇集會一併補記並自動去重；重溫本集會再次標記，是否受時間間隔限制取決於下方「允許重複標記」開關",
