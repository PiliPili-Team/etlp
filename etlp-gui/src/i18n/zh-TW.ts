...
export const zhTW: typeof zhCN = {
    ...
    sys_trakt_sync_note:
        "播放結束後自動同步：本集需有效觀看（約 20 秒以上、非誤點即退）才會上報，時長過短會跳過；看到約 80% 及以上標記為已觀看並寫入記錄，未達則依真實進度儲存為續播點（可在 Trakt 的 Up Next 跨裝置續播，注意 Trakt 要求進度高於 1% 才會儲存）；媒體伺服器中此前已看完的本季劇集會一併補記",
