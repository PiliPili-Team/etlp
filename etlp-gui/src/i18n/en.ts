...
export const en: typeof zhCN = {
    ...
    sys_trakt_sync_note:
        'Synced automatically when playback ends: the episode is only reported after a real watch (roughly 20s or more, not a momentary open); watching about 80% or more marks it watched and adds it to history, below that the real progress is saved as a resume point (resumable from Up Next on any device). Earlier episodes of the season already finished in your media server are backfilled too',
